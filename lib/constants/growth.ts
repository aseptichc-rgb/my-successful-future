...
/**
 * "오늘 잘한 일 3가지" 기록을 여는 최소 "역대 최고 연속일" — 실행 설계(7일) 다음 칸.
 * 기록은 매일 빈 칸 여러 개를 마주하게 하는 선택 입력이라, 필수 루틴(다짐 전사 · 목표
 * 체크)이 먼저 붙기 전에 열면 하루의 첫 화면이 숙제로 읽힌다. PLAN_UNLOCK_STREAK 와
 * 별개 정책이므로 값을 공유하지 않는다 — 하나를 조정할 때 다른 하나가 따라 움직이면
 * 해금 사다리(1 → 7 → 14)가 조용히 무너진다.
 */
export const WINS_UNLOCK_STREAK = 14;
