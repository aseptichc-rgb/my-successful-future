...
export const DAILY_QUOTA = {
  /**
   * "오늘의 또 다른 한마디" — 동기부여 카드 재생성(force=true) 호출.
  ...
  missionResponse: 5,
  /**
   * 클라이언트 관측 이벤트(/api/events) 기록. 정상 사용은 하루 열 건 안팎이라 넉넉히 두되,
   * 렌더 루프 버그·도배가 events 컬렉션을 채우지 못하게 상한을 건다.
   */
  events: 200,
  /** "만든 사람에게 한마디" 피드백 제출(/api/feedback). 카드는 평생 1회지만 시트 재전송 도배 방지. */
  feedback: 3,
  /**
