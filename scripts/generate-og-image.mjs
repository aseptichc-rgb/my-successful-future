...
/* ── 카피 ───────────────────────────────────────────────────────
   ...
   헤드라인 두 줄은 끊어 읽는 두 문장이 아니라 조건절 + 결과로 이어진 한 문장이다
   ("적으면 → 다가섭니다"). 첫 줄만 떼면 약속이 사라지고 둘째 줄만 떼면 무엇을 해야
   하는지가 사라진다 — 한쪽만 고치지 말 것.
   deck 은 그 약속이 왜 되는지(반복)를 말한다. 약속과 근거도 한 몸이다. */
export const HEADLINE_LINES = [
  { text: "꿈을 한 줄 적으면", accent: null },
  { text: "그 꿈에 한 걸음 다가섭니다.", accent: "한 걸음" },
];
export const DECK = "위대함은 매일의 작은 행동의 반복입니다.";

/* 썸네일은 피드에서 500px 안팎으로 줄어들므로 큰 글자가 이긴다. 두 줄이어도 세로가
   남아(밴드 약 305px / 본문 486px) 상한은 안전폭 쪽이다 — 넘기면 렌더 때 REJECT 로 잡힌다. */
